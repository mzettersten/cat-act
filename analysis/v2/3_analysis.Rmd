...
write_csv(category_labels,here(write_path,"catact-v2-label-responses.csv"))
...
Exploratory analysis looking at the effect of trial number and its interaction with training condition.

```{r}
#effect of trial number
sampling_data <- sampling_data %>%
  mutate(trial_number_c = trial_number-2)
#singular fit with within-subject trial number random slope, so removed
m <- glmer(sampling_choice_consistent_b~offset(logit(chance_consistent))+trial_number_c+(1|subject), data=sampling_data, family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)

#interaction between trial number and training condition
m <- glmer(sampling_choice_consistent_b~offset(logit(chance_consistent))+current_category_training_level*trial_number_c+(1|subject), data=sampling_data, family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
Anova(m,type="III")

#quick peek to understand the interaction
ggplot(sampling_data,aes(trial_number,sampling_choice_consistent_b,color=current_category_training_level))+geom_smooth(method="lm")
#increasing confirming choices in the Broad condition
```
...
```{r}
#effect of trial number
m <- glmer(sampling_choice_narrow_constraining_b~offset(logit(chance_narrowly_constraining))+trial_number_c+(1+trial_number_c|subject), data=sampling_data, family=binomial)
summary(m)

m <- glmer(sampling_choice_narrow_constraining_b~offset(logit(chance_narrowly_constraining))+current_category_training_level*trial_number_c+(1|subject), data=sampling_data, family=binomial)
summary(m)
Anova(m,type="III")

#quick peek to understand the interaction
ggplot(sampling_data,aes(trial_number,sampling_choice_narrow_constraining_b,color=current_category_training_level))+geom_smooth(method="lm")
#looks like things stay relatively consistent for the narrow condition, but constraining choices increase for the Intermediate Condition, while dropping in the Broad condition.
```
